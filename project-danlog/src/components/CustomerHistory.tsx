...
export const CustomerHistory = ({ customers }) => (
          ...
          <tbody className="text-[13px]">
            {customers.map((c) => (
              <tr key={c.id} className="group hover:bg-white transition-all">
                <td className="py-5 pl-10 border-b border-gray-50/50 font-black text-[#1A2B47]">{c.name}</td>
                ...
                <td className="py-5 text-center border-b border-gray-50/50">
                  <span className="bg-gray-50 text-gray-500 px-3 py-1 rounded-lg font-black text-[11px]">{c.loads}</span>
                </td>
